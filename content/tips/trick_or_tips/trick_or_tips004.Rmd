...
arr2[c(1,3)]
# with a row vector, we get 1rst row and the 3rd column cause we now are using
arr2[t(c(1,3))]
```
...
```{r}
# Let's subset `E`,`C` and `O` and `C` (again)
...
to specify what your function should return. There are programming langages where this instruction is mandatory, not in R! Check out the documentation `?return`:
...
`add_v()` and `add_v2()` are equivalent! So.. do we car? Well, you must bear in mind that whenever `return()` is encountered, the evaluation of the set of expressions within the function is stopped and so some time can be saved:
...
When using mathematical annotations, we sometimes need to do include the value
...
then variables evaluated will be the ones included in the environment passed as
argument `env` that could be a list.
