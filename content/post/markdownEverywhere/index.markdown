...
Broadly speaking, the idea of creating a language built on top of
...
[CoffeeScript](http://coffeescript.org/) that is built on top of JavaScript:
...
was built on top of [TeX](http://tug.org/) to ease the usage of its macros,
...
description of what a Markdown-to-HTML converter must do (harder than it sounds,
...
are built, that would be great (again it is not in the current Markdown landscape)!
...
use Markdown in different websites, different applications, you very likely
...
It is therefore not surprising that Markdown is supported by tons of websites and
...
with your colleagues on [Gitter](https://gitter.im/).

If you create your own website, you could easily find a way to edit content
with Markdown. Indeed, most famous Content Management Systems (CMS) such as
[Wordpress](https://en.support.wordpress.com/markdown/) supports Markdown as well
...
[Blogdown](https://bookdown.org/yihui/blogdown/) and use Markdown and R Markdown
...
different converters](https://github.com/rstudio/blogdown/issues/171).
...
Once again, Markdown is a lightweight markup language that emphasizes plain-text
...
(itself built on top of TeX :smile_cat:!), then why not using Markdown
...
all those languages (assuming we are talking about markup languages that are
concerned at least partially with plain-text readability). What if we could do
even more! What if we created an abstract representation of a markup language
...
So now that we have this fantastic tool thanks to John MacFarlane (I won't give much
details about Pandoc, which deserves a post it-self, if not more) we can perform
the conversion between many markup languages. Markdown then becomes a very useful
...
Remember that Markdown, irrespective of the flavor, only concerns a restricted number
of tags. So if you want further formats for your document you can simply use tags
from the markup language. So you can either stick to Markdown
and convert it into different formats or you can use Markdown and other tags
of the markup language you will convert your file into and then to any possible
conversion to other other markup language.
...
Given what I wrote above, a solution would be to use Markdown in combination
...
Fortunately, the needs of academic writing are covered by [Pandoc's Markdown](https://pandoc.org/MANUAL.html#pandocs-markdown)! So,
...
So basically thanks to Pandoc, Markdown it is suitable for academic writing.
There are even more advantages such as using [templates](https://pandoc.org/MANUAL.html#templates)
and benefit from [storing metadata of your manuscript directly in your file](https://pandoc.org/MANUAL.html#metadata-blocks) for instance through YAML header (another
potential post topic thanks to Clark Evans). Using Markdown may also require
...
[autocomplete-bibtex](https://atom.io/packages/autocomplete-bibtex) (to handle reference) and [document-outline](https://atom.io/packages/document-outline) (thanks to [@tpoi](https://twitter.com/tpoi) for the suggestion about the latter).
...
["Why scholars should write in Markdown"](http://blogs.harvard.edu/pamphlet/files/2014/08/markdownpost-amsart.pdf) by Stuart M. Shieber (more articles listed in the resource section).


A final note about reviews. I personally write my review in Markdown and convert
them in `pdf`. I then copy/paste the Markdown file and attach the `pdf` which
...
While I was writing this post I realized that just as Markdown is, resources
are everywhere! I though it would be useful to end this posts by providing a
personal selection of links.
...
There are specific editors such as [Abricotine](http://abricotine.brrd.fr/),
[Mou](http://25.io/mou/) or [Stackedit](https://stackedit.io/). Most of modern
code editors include specific library for highlight syntaxing and
preview. For instance [Atom](https://atom.io/) includes a [markdown preview](https://atom.io/packages/markdown-preview) a core
package and a quick search show that there are 30 packages (query on
