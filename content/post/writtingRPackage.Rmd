...
A couple of collegues and I have recently released a R package to the CRAN, [rcites](https://github.com/ibartomeus/rcites), a R client for the [Species+ API](https://api.speciesplus.net/). In the following lines, I would like to share my experience, few thoughts about R packages as well as valuables resources.
...
I like the way he presents a package, it is straightforward and crystal clear. Interestingly enough, it describes what a package **does**, not what a package **is**. Defining R package is actually harder than I thought! I remember the first time I've tried to characterize an R package, I asked myself a couple of questions and found the answers quite surprising:
...
Two points here. First R packages do many things: they can turn R into a power Geographic Information System or make you R session
crash each time you load it (see [trump](https://github.com/romainfrancois/trump))!

So **what is a R package?** Well, my own definition would be something like:

> a set of files that passes the command `R CMD INSTALL`

From that definition, one should understand that a R package does not necessarily performs any analysis correctly. It is not what it is and on the CRAN, nobody check if your package do it right, instead they check if your structure of file and your code are compliant to the numerous tests included in `R CMD INSTALL`!
...
This is another crucial question. In [his introduction](http://r-pkgs.had.co.nz/intro.html), Hadley Wickham emphasized that R packages are a standard way to organize your project which helps you saving time and ease the code sharing. Once your are used to R packages I garante you always organize your folder the same way and you dont loose your time finding a specific file.  (note that there is also other suggested way to organize your project with R without buiilding a package https://nicercode.github.io/blog/2013-04-05-projects/).
...
Sharing with collaborators, on a Github or simialr  very similar interect or sharing with many different people thanotopn g
...
all kinds of R extension material, such as data sets or documentation (manuals or books): because packages are versioned, keeping the extensions up-to-date can easily be achieved.

I think it is however not a definition and I think when we think about


> Package authors often seem to prefer copying code from other packages than depending on these, or collaborate to develop a common infrastructure. If packages provide different solutions to similar tasks, they will evidently rather compete than cooperate through hierarchical dependencies.
...
[A curration of package](https://awesome-r.com/)
...
I personally use the [devtools]() packages developed by Wicham and colleagues that make you life really easy with packae. and basically explianed with the book. By the way [version 2.0]([5:53 PM]
