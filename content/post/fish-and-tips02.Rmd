---
title: Fish and tips 002 {R}
author: [kevin]
date: 2017-11-12
tags: [R,tips]
tweet: Second post on Fish and Tips series
draft: false
output:
  blogdown::html_page:
    toc: true
...
```{r trick, echo=FALSE, results='asis'}
source('../../static/Rscript/trickTweak.R')
```
...
## The `drop` argument on the `[]` operator   
...
=======
Many researchers (it is especially TRUE in eclogy) uses R and write paper and
carry out analyses using R for their research. One cones the time of citing the
package I guess they wonder how to citethe package. However authors of
>>>>>>> dev
...
=======
know and use more and more packages. You migh even come to the point
>>>>>>> dev
...
=======
Here I use the function `extract()` fron the magrittr package that act as `[]`
>>>>>>> dev
...
=======
to `:` as for using the namespace, you use three! Let's see the code cource of one
of this function of `knitr` (again version 1.17):
>>>>>>> dev
...
=======
Interesting, isn't it! And actually in this packages they are 103 exported
functions and 425 not-exported. Below are presented few examples of exported
functions followed by not-exported ones.
>>>>>>> dev
...
values always puzzles me! One I specially do like is that values on the y-axis
are perpendicular...
