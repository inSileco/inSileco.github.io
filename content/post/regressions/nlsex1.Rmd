...
$$T(t) = T_0 - (T_{\inf} - T_0)\exp(-k*seqt)$$

### Goal and data

The goal here is to use `nls()` (Nonlinear Least Squares) to find $k$ and $T_\{inf}$. For the sack of
clarity I'll simulate the data, i.e. I use the saturation curve with know parameter,
then I add some noise (here a [white noise](https://en.wikipedia.org/wiki/White_noise)):
...
```{R}
res <- nls(sim ~ Tinf - (Tinf - 10)*exp(-k*seqt),
      data = simdata, start = list(Tinf = 1, k = .01))
```
