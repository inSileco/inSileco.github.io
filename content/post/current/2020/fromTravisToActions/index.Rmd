---
title: "Continuous integration for R projects: from Travis CI to GH actions  step by step"
author: [kevin]
reviewer: [marieh]
date: 2020-11-23
...
<https://travis-ci.org> and turn off the jobs described above. To do sio, I looked for  `inSilecoMisc` in my list of projects and switched it off (see screenshot below).
...
The workflow I ended up using, [R-CMD-check.yaml](https://github.com/inSileco/inSilecoMisc/blob/master/.github/workflows/R-CMD-check.yaml) is based on one of the template file available in [`usethis`](https://CRAN.R-project.org/package=usethis)
...
There are a few lines that require some explainations. First, I decided to build this on Ubuntu (again, this is arbitrary). Second, the following line: 
...
For this, one needs to generate a [pair of ssh keys](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent#generating-a-new-ssh-key). On my Debian machine, I used the following in 
my terminal: 
...
Again I chose `ACTIONS_DEPLOY_KEY` for its clarity, but the name does not matter as long as the correct variable name is used in [R-CMD-check.yaml](https://github.com/inSileco/inSilecoMisc/blob/master/.github/workflows/R-CMD-check.yaml).
...
Everything is working fine now :fireworks:, check out [`inSilecoMisc`](https://github.com/inSileco/inSilecoMisc)! Hope this could be of help :smile:!
