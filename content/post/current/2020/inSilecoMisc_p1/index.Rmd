...
it is meant to stay at experimental stage! I like having `inSilecoMisc` on
...
least, I hope so :smile:). I have even included some of the matured functions in
the research compendium of recent manuscripts (see the following <i class="fa
fa-github" aria-hidden="true"></i> repositories:
...
As an ecologist, I work with multiple datasets, and I often need to name/rename
...
2. `n`: the number of characters to be added or used to produce fix length strings;
...
cat(loremIpsum())
```
but also allow the user to choose a number of words:
...
want to extract the second word in both sentences `loremIpsum(18)` an `be or not to be`, all I have to do is:
...
and if I want specific word positions, I can pass a numerical vector:

```{R}
keepWords(c(loremIpsum(18), "be or not to be"), c(1:4, 12:16))
```

As you may have notted, `NA`s are added when the position selected does not
exist. This behaviour could be useful but also annoying! Fortunately, an
...
Note that all punctuations signs will be removed (this can be changed with argument `split_words `)!


There are two other functions that work similarly: `keepLetters()` and `keepInitials()`. The former select letters instead of words.
...
[data-flair](https://data-flair.training/blogs/r-string-manipulation/)) and,
obviously, the documentation of packages that do such manipulations (e.g. `r
url_pkg_cran("stringi")`).
