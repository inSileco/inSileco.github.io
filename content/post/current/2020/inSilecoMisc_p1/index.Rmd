...
`.Rprofile`, I created a package available on GitHub. `inSilecoMisc` is
therefore a set of miscellaneous functions, just as other R packages (e.g.
[Hmisc](https://github.com/harrelfe/Hmisc)) but as I frequently change its API,
it is meant to stay in a experimental! I like having `inSilecoMisc` on GitHub in
such a experimental mode: it gives a large degree of freedom to experiment new
ideas, new functions, new packages, etc., but still, any version of the code is
available and easy to install! Despite the "API instability", the overall
quality of the package is constantly improving (at least, I hope so :smile:). I
have even included some of the matured functions in the research compendium of
recent manuscripts (see the following <i class="fa fa-github"
aria-hidden="true"></i> repositories:
[HomogenFishOntario](https://github.com/McCannLab/HomogenFishOntario)
and
...
re-wrote some functions, improved the documentation of other and added new
features. I also changed the way the [pkgdown
website](https://insileco.github.io/inSilecoMisc/) is deployed by
[Travis](https://travis-ci.org/github/inSileco/inSilecoMisc) (I followed the
steps described in this
[gist](https://gist.github.com/gaborcsardi/68960fb01bec74d89611bb9c2b0b7e5a) by
[Gábor Csárdi](https://github.com/gaborcsardi) -- very clear and extremely
useful). In what follows, I introduced several functions included in
versions 0.4.0. Note that as `inSilecoMisc` is on GitHub, an easy way
to install `inSilecoMisc` is to use
...
As an ecologist, I work with multiple data sets, and I often need to name/rename
a bunch of files. One function I often use to do so is `sprintf()`:

```{R}
sprintf("file_%02d", 1:10)
```

I originally designed `adjustStrings()` to adjust the number of 0 to a string in
a similar fashion:
...
1. `x`: the vector of strings to be adjusted;
...
By default behavior of `adjustStrings()` use `n` as a constrain on the length
so I we
...
and extra allows to use any charcater strng
...
and "align" allows use how sting should be aligned `x`
...
If `add = TRUE`, then exactly `n` extra characters are added to strings (and so they may have different lengths):
...
All these features allow the user to create various handy functions. For
instance, when running a long script, it is convenient to create recognizable
piece of information reporting where we at and `adjustStrings()` can help!
...
`keepWords()` allows the user to select words based on their position in
...
cat(loremIpsum())
```
but also allow the user to chose a number of words:

```{R}
loremIpsum(18)
```

Note the escape characters (`\n`)! Now let's focus on `keepWords()`. Assuming I I need the second word, all I have to do is:
...
Sanchez](https://www.gastonsanchez.com/). There are also various blog posts on this topic (for
instance
[http://uc-r.github.io](http://uc-r.github.io/string_manipulation_baseR/) or on
[data-flair](https://data-flair.training/blogs/r-string-manipulation/)) and, obviously, the documentation of packages that do such manipulations (e.g. `r url_pkg_cran("stringi")`).

This concludes the first part of this post, additional features of the  `inSilecoMisc` package will be introduced the second part of this post :tada:!
