...
case columns providing the coordinates must be included abd the user must know
...
The point dataset we work with below represents sampling points of a monitoring
...
objects using `st_as_sf()`. We need to specify the columns containing the
coordinate values (here `LATITUDE` and `LONGITUDE`) and the CRS (here we know
that the datum is WGS84 and the [EPSG](http://www.epsg.org/) is 4326 from the [metadata](http://donnees.ville.montreal.qc.ca/dataset/rsma-points-d-echantillonnage-ruisso/resource/adad6c48-fb48-40fc-a031-1ac870d978b4)).
