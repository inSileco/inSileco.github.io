...
Similarly, if we know that this particular species has an average death rate of
$d$ individual every year, $N_{t+1}$ would be equal to the number of individuals
at time $t$ plus their offsprings minus the annual number of dead individuals:
...
pop <- data.frame(pop1 = geomMod(10, .5, 1:5),
                  pop2 = geomMod(10, 1, 1:5),
                  pop3 = geomMod(10, 1.5, 1:5))
...
scale by using the exponentiation, given that we values > 0:
...
### Obtenir le modèle
<p style="color:red;">DONE, voir les changements, c'est ok?</p>

<p class="comment2">Oui</p>

<div class="comment2"> Oui, 2 commentaires:

1. avant le "Similarly, " qui nous emmène vers l'approche qui inclut le nombre
de morts je releverais le fait que pas de morts sont inclus dans l'autre,
les individus sont éternels dans cette formulation!

2. "This is however rarely (read *never*)
the case in real life."  (l.256) Je pense que tu pourrais dire un peu plus pourquoi!

</div>

Je pense qu'il faudrait que tu etablisses le modele avec les phrase que tu as
écrites au lieu de le donner puis le décire. Tu a tout écris c'est juste
qu'il faut inverser l'ordre!

Tu pourrais dire qu'entre 2 années (entre t+1 et t) il y a les individus de
l'années suivante et leur descendance, si b est la descendance moyenne par individu alors:

$$N_{t+1} = N_{t} + bN_{t} = (1 + b)N_{t}$$

et donc $\lambda = (1+b)$. Ou alors, l'option que je préfère: d'une année a l'autre il y a un taux de de mortalité moyen $d$ et un taux de naissance moyen $b$:

$$N_{t+1} = N_{t} + bN_{t} - dN_{t} -  = (1 + b - d)N_{t}$$

et donc $\lambda = 1 + b - d$. C'est en fait comme ça qu'on démontre comment
passer du discret au continu (la deminstration n'est pas trop méchante).

### Remarque sur le log

En fait il serait important de noter à quelque part que:

$$N^t = \exp(t\log(N))$$

<p style="color:red;">Est-ce que c'est ok comme ça?</p>
<div class="comment2"> Oui! </div>


Pourquoi? Parceque:

- pas besoin d'utiliser de `spline` pour les prédictions
<p style="color:red;">Pourquoi? Parce qu'on peut utiser la fonction exponentielle?
Si oui, j'attendrais quand même au post sur le modèle exponentiel avant de l'introduire</p>
<p class="comment2"> Fair enough </p>


- Ca permet de comprendre pourquoi il faut utiliser une moyenne géométrique (qui
  est en fait une moyenne arithmétique sur les log) et ça explique du meme coup la trick du post de stack overflow.
<p style="color:red;">Je comprends le tricks, mais je comprends pas pourquoi ça
nous permettrait de comprendre qu'il faut utiliser la moyenne géométrique plutôt
que la moyenne arithmétique?</p>

<p class="comment2"> Ouep t'as raison ça explique pas. J'avoue que je vois pas
une façon simple de dire pourquoi si ce n'est ce qui est déjà écrit. Il faudrait
embarqué sur les propriétés des estimateurs... Ça va nous emmener trop loin. On
garde ça en tete pour une série sur les stats qui peut etre ce fera un jour. </p>


### Figures

- Je préfère utiliser `las = 1` (voir le post de trick) pour afficher les textes
horizontalement
<p style="color:red;">DONE sauf pour `matplot()`</p>

- sur la 2eme figure j'aurai mis les vrai valeurs de $\lambda$.
<p style="color:red;">DONE</p>

- sur les figures avec les taux vairables, je mettrais scenario minimu et scenario maximum!
<p style="color:red;">DONE, mais on voit moins la différence entre la moyenne arithmétique et géométrique</p>

<p class="comment2">  Tu pourrais les enlever et mettre une envelop gris clair!!
Regardes la funtion "envelop" dans graphicsutils </p>



### New comments

<p class="comment2">  pas dur de ce que tu voulais dire après "exponentiation,":
"given that we values > 0"??? </p>
