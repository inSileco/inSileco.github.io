...
rmarkdown::render('./metanetwork.rmd', 'html_document')
...
enjoy using all that much (*e.g.* [this](https://www.r-graph-gallery.com/hierarchical-edge-bundling/)).
...
# Individual network nodes
  drivers <- data.frame(network = 'Drivers', name = paste0('driver_',1:15), stringsAsFactors = F)
  species <- data.frame(network = 'Species', name = paste0('species_',1:30), stringsAsFactors = F)
  actions <- data.frame(network = 'Actions', name = paste0('action_',1:9), stringsAsFactors = F)
  managers <- data.frame(network = 'Managers', name = paste0('manager_',1:9), stringsAsFactors = F)
  beneficiaries <- data.frame(network = 'Beneficiaries', name = paste0('beneficiary_',1:9), stringsAsFactors = F)
...
The first function we need will be used to generate the x and y coordinates of a circle for a given angle and
circle radius, which we can obtain with some circle trigonometry.
...
boxGroup <- function(metanetwork, rad1 = .95, rad2 = 1, colBox = NULL, names = NULL, colNames = NULL, addNames = T, ...) {
  ...
  for(i in 1:nrow(metanetwork$networkGroup)) {
    ...
    polygon(rbind(a, b, a[1,]), col = colBox[i], ...)
...
```{r plotLinks, fig.width = 7, fig.height = 7, fig.align = 'center'}
  ...
  points(metanetwork$nodes$x, metanetwork$nodes$y, pch = 20, cex = 2)
  plotLinks(metanetwork, cols = '#000000')
```
