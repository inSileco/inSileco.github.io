...
*Last update*: September 14, 2017 -- Fix paths to icon files.
...
All of these frameworks propose either a `.ttf` (TrueType Font) or a `.otf` file (OpenType Font) (or both). These files make our life very easy and we can make a direct **link** with a [previous blog post](./HylianStyle.html).
...
We first need to download the 3 fonts we will be using. Links to retrieve the
font are available right below and the following code chunk describes how
use the R to get these font.
...
Below we download and put the font in a local folder:




If you have downloaded the font by clicking, you can either install them on
your computer (there is most likely an application on your computer to do so) or
store them in a dedicated folder (which path is `pth` for the rest of the post).


```{r downloads}
pth <- "./AcademiconsInR_files"
...
for (i in 1:3){
  download.file(urls[i], destfile=paste0(pth, '/', gsub(urls[i], pattern = '.*/', replace = '')))
}
```
...
##-- checke the font families available:
font_families()
```
...
all numbers from 00 to 35 (version 1.8.0). To use them, we create a
