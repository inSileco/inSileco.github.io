...
*Last update*: November 11, 2017 -- Fix paths to icon files.
...
make a direct **link** with a [previous blog post](/content/post/HylianStyle.html).
...
##-- download the fonts
for (i in 1:3){
  download.file(urls[i], destfile=paste0(dr, basename(urls[i]), method='libcurl'))
}
```
...
```{r addfonts}
font_add(family = 'academicons', regular = paste0(dr,'academicons.ttf'))
font_add(family = 'FontAwesome', regular = paste0(dr,'FontAwesome.otf'))
font_add(family = 'ionicons', regular = paste0(dr,'ionicons.ttf'))
...
(<i class="fa fa-meetup" aria-hidden="true"></i>) in version 4.7.
...
and [Github](https://guangchuangyu.github.io/emojifont/).
