...
You can simply click on the links below to get them one by one or [here](/assets/AcademiconsInR/fontIcons.zip)
to get all on them.
...
Once downloaded, install them on your computer (there is most likely an
application on your computer to do so but note that it is also possible to
store the fonts in a dedicated folder and then provide the path to use them
with `showtext`. See the `font.paths()` function.



## Loading the fonts

To activate the facilities `showtext` provide, it only requires `showtext.auto()`
...
Then, add `academicons.ttf`, `FontAwesome.otf` and `ionicons.ttf` fonts to your
session with the `font.add()` function:

```{r addfont}
pth <- paste0(getwd(),'/../../static/assets/AcademiconsInR/fonts')
font.add(family = 'Academicons', regular = paste0(pth,'/Academicons.ttf'))
font.add(family = 'FontAwesome', regular = paste0(pth,'/Academicons.ttf'))
font.add(family = 'Ionicons', regular = paste0(pth,'/Academicons.ttf'))
```

Here, things are going to be a little bit trickier than they were in the
[Hylian blog post](./HylianStyle.html), as we should first locate the
