...
library(raster)
dir.create("data")
can1 <- getData("GADM", country = "CAN", level = 0, path = "data/")
can2 <- getData("GADM", country = "CAN", level = 1, path = "data/")
```
...
can2_sf <- st_as_sf(can2)
```

Retrieve Quebec polygon

```{r getQc, message = F}
qc <- can2_sf[can2_sf$NAME_1 == "Québec",]
```

Let's look at what we have

```{r map_1, message = F}
plot(st_geometry(can1_sf), lwd = 1.5, col = "grey85")
plot(st_geometry(qc), lwd = .5, col = "blue3", add = T)
```
It takes a while to plot because there is a lot of unnecessary details, so we
can simplify the shape of the polygons using `st_simplify()`.

```{r simplify, message = F}
can1_simple <- st_simplify(can1_sf, dTolerance = 0.01, preserveTopology = TRUE)
qc_simple <- st_simplify(qc, dTolerance = 0.01, preserveTopology = TRUE)
```
...
```{r points}
# Sample random points from Quebec
...
# myblu <- '#6da6c2'
# mygre <- 'grey50'
# mypal <- colorRampPalette(rev(brewer.pal(11, "RdYlBu")))
# #
# plot0(c(-96,-74), c(41,57), fill=myblu)
# image(temp_crop, add = T, col = mypal)
# plot(st_geometry(can1_simple), add = T, col ='#b5cfbd', border = mygre)
# plot(st_geometry(qc_simple), add = T, col = NA, border ='grey15', lwd =1.4, add = T)
# plot(st_geometry(sample_pts), cex = sample_pts$richness)

# legend
# par(mar = c(3.2, 5, .5, 5), mgp = c(2,.5,0))
# val <- range(values(temp_crop), na.rm =T)
