...
You'll notice this is a bit different and a bit further than the `gdistance` estimates. This is likely mostely because of the difference in resolution (and somewhat because of cell shape). If your just calculating a handful of distances such as in this example, `gdistance` is probably faster. But if you're doing millions, `igraph` is typically more effective since much of the work is done 'upfront' when you create the graph. It would also be possible to create an igraph network from a gdistance transition raster which would combine the best of both packages.
