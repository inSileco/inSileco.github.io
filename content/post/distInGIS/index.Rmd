...
Many other exists that can be used with different coordinate systems.



# Geographical distances and the problem of heterogeneity

Let's choose a concrete example: let me introduce Herbert the lobster. Herbert is a wild lobster living in the bay of Sept-Îles (Québec, Canada) and we, as researchers, are interested in knowing the connectivity of its actual distribution to other potential living places close by, in the context of predicting future distribution under a scenario of habitat loss due to human perturbation.
...
We will work with the `sf` package, as described in the previous [inSileco GIS series](/tuto/rinspace/rinspace_homepage/) (*Pebesma 2018*).
...
```{r plot1, echo=FALSE, warning=FALSE, fig.align="center", eval = TRUE}
par(mar = c(3, 3, 1, 1))
...
```{r plot2, echo=FALSE, warning=FALSE, fig.align="center", eval = TRUE}
par(mar = c(3, 3, 1, 1))
plot(st_geometry(bay), axes = T, main = "", col = "#E2E2E2", border = "#C1C1C1")
plot(coords, cex = 2, pch = 16, add = T)
text(x = st_coordinates(coords)[,"X"],
...
1. forbidden cells will have a value of 0;
2. allowed cells will have a value of 1 (each cell has the same "cost", meaning no weighting have been applied for particular paths).

Note that the calculation is highly dependent on the **raster resolution**. A better resolution will bring better results, but at the cost of computing resources.
...
```{r plot3, echo=FALSE, warning=FALSE, fig.align="center", eval = TRUE}
par(mar = c(3, 3, 1, 1))
plot(raster_bay)
```
...
The `geoCorrection()` function is needed to apply a correction to the transition matrix so that map distortion is taken into account (this may be more or less relevant depending on the coordinate reference system used).
...
```{r plot4, echo=FALSE, warning=FALSE, fig.align="center", eval = TRUE}
par(mar = c(3, 3, 1, 1))
plot(st_geometry(bay), axes = T, main = "", col = "#E2E2E2", border = "#C1C1C1")
plot(coords, cex = 2, pch = 16, add = T)
text(x = st_coordinates(coords)[,"X"],
...
## Bonus: Least Cost Distances with Hexagons and `igraph`

The `gdistance` package is excellent, but not particularly flexible. You may need a custom workflow if you have very large datasets or you want to use a hexagonal grid (i.e. you don't want to worry about chessboard moves!). With this method, we will create a grid that covers

Let's create another grid, but this time, let's make it hexagonal. For this example, the cell centroids are 250 m apart (in any of the 6 directions!), but you could increase the resolution if you don't mind waiting a bit. As you can see from the map below, such a high resolution may ignore the presence of some smaller islands.
...
Now that we have our basic grid, we need to create an `igraph` network. The network in this case will represent neighbouring (*i.e.* right next to one another) cells which Herbert could move through. We must first create an edgelist (*i.e.* list to 'from' and 'to' cell id's that represent neighbours) which then gets converted into a network.

```{r, eval = TRUE}
edgelist <- as.matrix(as.data.frame(st_intersects(hex_bay))) # yes, the as.matrix and as.data.frame is a bit silly, but required to get the data in the right format
head(edgelist)
```
...
Yes, this plot no longer looks like the bay because we have abstracted out the actual coordinates and are looking only at the distances between cells. In this particular example we know that the distance between each neighbouring cell is 1000 m since we defined this when we created the grid. so the 'weight' (*i.e.* distance) for each edge (*i.e.* link between neighbours). At this phase you could also incorporate 'fancier' weights that incorporate habitat resistance (*i.e.* the influence of habitat type on the facilitation or prevention of movement) to cacluate isolation by resistance. But for this example, let's keep it simple.

```{r, eval = TRUE}
E(gr)
E(gr)$weight <- 250    #setting the weight for ALL edges, alternatively could supply a vector here
is.weighted(gr)
```
...
ggplot(bay)+
  geom_sf(size=1)+
  geom_sf(data=hex_bay,col='blue',fill='transparent') +
  geom_sf(data=hex_bay[whereisHerbert],col='red',fill='red') +
  geom_sf_label(data=coords, aes(label=station),nudge_y = 1000)+
  theme_bw()


distances(gr,whereisHerbert[1],whereisHerbert[-1])
```

You'll notice this is a bit different and a bit further than the `gdistance` estimates. This is likely mostly because of the difference in resolution (and somewhat because of cell shape). If your just calculating a handful of distances such as in this example, `gdistance` is probably faster. But if you're doing millions, `igraph` is typically more effective since much of the work is done 'upfront' when you create the graph. It would also be possible to create an igraph network from a gdistance transition raster which would combine the best of both packages.
