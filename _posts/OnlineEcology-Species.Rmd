...
```{r addStatus, echo=FALSE, results="asis", message=FALSE}
  ...
  readYAML("OnlineEcology-Species.Rmd")
```
...
the tools that are at our disposal. In a series of post about online ecology,
we will find out exactly just how far these tools allow us to delve into the
ecology of the species that interest us.
...
2. Community spatial distribution (*under development*)
3. Biotic interactions (*under development*)
...
to extract informations for other species in other habitats.*
...
We can also extract taxonomic informations using the package [`taxize`](https://ropensci.org/tutorials/taxize_tutorial.html).
...
# List sdmpredictors datasets
    kable(sdmpredictors::list_datasets())
```
...
# Data description
    kable(sdmpredictors::get_layers_info(layers)$current[,1:4])
...
```{r, modelEval}
# Model evaluation
    kable(biomod2::get_evaluations(SDM))
```
